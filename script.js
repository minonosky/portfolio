...
document.querySelectorAll("[data-copy-email]").forEach((button) => {
  ...
  const confirmation = button.querySelector(".copy-confirmation");
  ...
  button.addEventListener("click", async () => {
    ...
    confirmation.textContent = copied ? "[copied]" : "[failed to copy]";
...
if (robloxStats) {
  ...
  fetch("/api/roblox-stats")
    ...
    .catch(() => {
      // Keep the unavailable placeholders visible if live statistics cannot be loaded.
    });
}
